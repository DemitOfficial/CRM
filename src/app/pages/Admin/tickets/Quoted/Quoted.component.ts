...
@Component({
  ...
  styleUrls: ['./Quoted.component.scss']
})
...
debugger;
  ...
  ngOnInit() {
    ...
    this.breadCrumbItems = [{ label: 'Tickets' }, { label: 'Quoted Tickets', active: true }];
    this.Alltickets();
    setInterval(() => {
...
this.router.navigate(['/admin/userdetail']);
  ...
  {  
    ...
    this.ticketservice.getAlltickets().subscribe((next:any)=>{
      ...
      this.spinner.hide();
      this.Quotedtickets=this.ticketdata.filter(x=>x.ticketType.ticketName=="Quoted Parts");
